...
function statusFromPercent(p) {
  if (p >= 90) return { label: "Stable", tone: "good" };
  if (p >= 60) return { label: "Attention", tone: "warn" };
...
const PSPCL = [
  { d: "Sep 05", restored: 25 },
  { d: "Sep 06", restored: 53 },
  { d: "Sep 07", restored: 56 },
  { d: "Sep 08", restored: 74 },
  { d: "Sep 09", restored: 74 },
  { d: "Sep 10", restored: 75 },
  { d: "Sep 11", restored: 75 },
  { d: "Sep 12", restored: 75 },
  { d: "Sep 13", restored: 75 },
];

const PATIENTS = [
  { d: "Sep 05", patients: 959 },
  { d: "Sep 06", patients: 915 },
  { d: "Sep 07", patients: 540 },
  { d: "Sep 08", patients: 632 },
  { d: "Sep 09", patients: 211 },
];
...
function Icon({ name }) {
  ...
  switch (name) {
    case "droplet":
      return (
        <svg {...common} viewBox="0 0 24 24">
          <path d="M12 2s7 8.14 7 12a7 7 0 1 1-14 0c0-3.86 7-12 7-12" />
        </svg>
      );
    case "users":
      return (
    ...
    case "skull":
      return (
        <svg {...common} viewBox="0 0 24 24">
          <path d="M12 2a9 9 0 0 0-9 9c0 3.09 1.64 5.79 4.09 7.28L7 22h10l-.09-3.72A8.99 8.99 0 0 0 21 11a9 9 0 0 0-9-9Z" />
    ...
    <div className="stat-card">
      ...
      <div className="stat-main">
        <div className="stat-big">
          {fmt(primary.part)}
          <span className="stat-small">/{fmt(primary.total)}</span>
        </div>
        <div className="stat-sub">{primary.label}</div>
        <div className="stat-bar">
          <div className="stat-fill" style={{ width: `${p}%` }} />
        </div>
      </div>
...
/* --------------------------- Charts (pure SVG) ----------------------------- */
function YScale(domainMax, h, top = 10, bottom = 24) {
  const innerH = h - top - bottom;
  return (v) => top + (1 - v / (domainMax || 1)) * innerH;
}
function XScale(n, w, left = 36, right = 12) {
  const innerW = w - left - right;
  return (i) => left + i * (innerW / Math.max(1, n - 1));
}
        ...
        <div className="chart-legend">
          {legend.map((l, i) => (
            <div key={i} className="legend-row">
              <span className={`swatch ${l.className || ""}`} />
              {l.label}
            </div>
          ))}
        </div>
...
function LineChartPower() {
  ...
  const W = 460,
    H = 190;
  const x = XScale(data.length, W);
  const y = YScale(Math.max(target, ...data.map((d) => d.restored)), H);
  ...
  return (
    <svg
      ...
      className="svgChart"
      role="img"
      aria-label="Power restoration line chart"
    >
      {ticks.map((t, i) => {
        const yy = y(t);
        return (
          <g key={i}>
            <line x1="36" x2={W - 12} y1={yy} y2={yy} className="gridline" />
            <text x="30" y={yy + 3} className="tick">
              {Math.round(t)}
            </text>
          </g>
        );
      ...
      {data.map((d, i) => (
        <circle key={i} cx={x(i)} cy={y(d.restored)} r="3" className="marker" />
      ))}
    </svg>
  );
}
...
function BarChartPatients() {
  ...
  return (
    <svg
      ...
      className="svgChart"
      role="img"
      aria-label="Patients attended bar chart"
    >
      {[0, 0.33, 0.66, 1].map((f, i) => {
        const val = f * Math.max(...data.map((d) => d.patients));
        const yy = y(val);
        return (
          <g key={i}>
            <line x1="36" x2={W - 12} y1={yy} y2={yy} className="gridline" />
      ...
      {data.map((d, i) => {
        const x = left + i * (innerW / data.length) + 4;
        return (
          <g key={i}>
            <rect
              x={x}
            ...
            <text
              ...
              y={H - 2}
              className="xtick"
              transform={`rotate(-35 ${x + bw / 2} ${H - 2})`}
            >
              {d.d}
            </text>
          </g>
...
function ComboChartWater() {
  ...
  const bw = (step - 8) / 2;
  const maxVal = Math.max(...affected, ...restored);
  const y = YScale(maxVal, H, top, bottom);
  return (
    <svg
      viewBox={`0 0 ${W} ${H}`}
      className="svgChart"
      role="img"
      aria-label="Water schemes affected vs villages restored"
    >
      {[0, 0.33, 0.66, 1].map((f, i) => {
        ...
        return (
          <g key={i}>
            ...
            <text
              ...
              y={H - 2}
              className="xtick"
              transform={`rotate(-35 ${x0 + step / 2} ${H - 2})`}
            >
              {d}
            </text>
          </g>
...
function Dashboard() {
  return (
    <div className="charts-grid">
      <ChartCard
        title="Power Supply Restoration (Villages)"
        legend={[
          { label: "Villages with Power Restored", className: "lgA" },
          { label: "Target", className: "lgRef" },
        ]}
      >
        <LineChartPower />
      </ChartCard>
      <ChartCard
        title="Medical Relief — Patients Attended"
        legend={[{ label: "Patients (per day)", className: "lgB" }]}
      >
        <BarChartPatients />
      </ChartCard>
      <ChartCard
        title="Water Supply — Schemes Affected vs Villages Restored"
        legend={[
          { label: "Schemes Affected", className: "lgA" },
          { label: "Villages Restored", className: "lgB" },
        ]}
      >
        <ComboChartWater />
      </ChartCard>
      <div className="chart-note">
        Figures are operational and subject to validation.
      </div>
    </div>
  );
}
  ...
  return (
    <div className="card" id="flood-gallery">
      <div className="card-title">
        <Icon name="pin" /> Flood Situation Gallery
      </div>
      <div className="muted">
        Click a tile to expand. Use ←/→, Esc to close.
      </div>
    ...
    <div className="page">
      ...
      <section className="band">
        <div className="content hero">
          ...
          <p className="lede">
            Heavy monsoon rains caused localised flooding in riverine belts.
            This portal provides verified updates, response metrics, and secure
            channels for NGOs to collaborate with district authorities.
          </p>
          <div className="ctas">
            <a
              className="btn primary"
              href="#cta"
              aria-label="See what you can do"
            >
              SEE WHAT YOU CAN DO →
            </a>
            <a
        ...
        <div className="content">
          <div className="info-grid">
            <div className="card">
              ...
              <ul className="list">
                <p>
                  Heavy and sustained rainfall, along with overflows from the
                  Ravi River and Sakki Nallah, caused significant flooding
                  across the border districts of Amritsar. The disaster affected
                  196 villages, impacting over 136,000 people. The report
                  confirms 8 human lives were lost and multiple breaches in the
                  river and nullah embankments. The primary focus of the
                  government's response has been on restoring essential services
                  and providing immediate relief to the most vulnerable areas.
                </p>
              </ul>
            </div>
            <div className="card">
              ...
              <ul className="list">
                <li>
                  <strong>State Control Room:</strong> 1070 (24×7)
                </li>
                <li>
                  <strong>District Helpline:</strong> 112 (Emergency)
                </li>
                <li>
                  <strong>Relief & Shelter:</strong> Contact BDPO / Tehsildar,
                  nearest SDO (PWD/Water Supply).
                </li>
                <li>
                  <strong>Health:</strong> Mobile medical units; ambulance 108;
                  preventive kits at sub-centres.
                </li>
              </ul>
            </div>
          </div>
          ...
          <div className="info-grid" style={{ marginTop: 16 }}>
            <div className="card">
              <div className="card-title">
                <Icon name="chart" /> Key Government Actions (Recent)
              </div>
              <ul className="list">
                <p>
                  The report details a coordinated, multi-departmental response
                  to the flood situation:
                </p>
                <li>
                  <b>Power & Water:</b> The PSPCL (Punjab State Power Corporation
                  Limited) successfully restored electricity to all 75 of the
                  affected villages under its jurisdiction. The Water Supply and
                  Sanitation Department has been actively working to restore
                  potable water, with ongoing chlorination efforts in affected
                  villages.
                </li>
                <li>
                  <b>Medical Relief:</b> The Health Department established numerous
                  medical camps, attending to hundreds of patients with common
                  post-flood ailments like fever and diarrhea. They also
                  distributed essential medicines, conducted fogging and
                  spraying to prevent mosquito-borne diseases, and actively
                  monitored for potential outbreaks.
                </li>
                <li>
                  <b>Animal Husbandry:</b> To assist the livestock community,
                  veterinary teams were deployed to treat thousands of affected
                  animals and distribute fodder. The report notes that 40
                  deceased animals were reported and buried to prevent the
                  spread of disease.
                </li>
                <li>
                  <b>Shelter & Relief:</b> Multiple relief centers were opened to
                  provide shelter to displaced civilians and their cattle.
                  Departments worked together to distribute dry ration kits,
                  ready-to-eat meals, blankets, and sleeping mats.
                </li>
              </ul>
            </div>
            <div className="card">
              ...
              <ul className="list">
                <p>
                  The report includes a section specifically for NGOs and
                  external parties. The key message is to ensure all efforts are
                  coordinated to avoid duplication and maximize impact.
                </p>
                <li>
                  <b>Registration:</b> Organizations interested in helping are required
                  to register their support for key services such as water and
                  sanitation, shelter, health services, animal care, and
                  logistics.
                </li>
                <li>
                  <b>Coordination:</b> It is mandatory to coordinate all activities
                  with the District Control Room before deploying any teams or
                  resources. This ensures that help is sent to the areas with
        ...
        <div className="content">
          <div className="card" id="gallery">
            ...
            <FloodGallery
              images={[
                {
                  src: "/floods/flood%20images_page-0001.jpg",
                  caption: "Flood situation image 1",
                },
                {
                  src: "/floods/flood%20images_page-0002.jpg",
                  caption: "Flood situation image 2",
                },
                {
                  src: "/floods/flood%20images_page-0003.jpg",
                  caption: "Flood situation image 3",
                },
                {
                  src: "/floods/flood%20images_page-0004.jpg",
                  caption: "Flood situation image 4",
                },
              ]}
            />
          </div>
      ...
      <section className="band">
        <div className="content">
          <div id="dashboard" className="card" style={{ paddingTop: 12 }}>
            ...
            <div className="muted">
              Daily progress indicators (illustrative; align with official
              time-series).
            </div>
            <Dashboard />
          </div>
      ...
      <section className="band alt" id="cta">
        <div className="content">
          <div className="card">
            <div className="card-title">
              For Interested NGOs (Official Coordination)
            </div>
            <div className="muted">
              Register support for water & sanitation, shelter, health, animal
              care and logistics. Coordinate with the District Control Room
              before deployment.
            </div>
            <div className="ctas" style={{ marginTop: 12 }}>
              <a className="btn primary" href="/ngo/login">
                Sign In / Register
              </a>
            </div>
          </div>
      ...
      <footer className="footer">
        © {new Date().getFullYear()} Punjab Flood Relief — Official
      </footer>
    </div>
  );
}
