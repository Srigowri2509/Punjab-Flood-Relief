...
/**
 * Punjab Flood Relief — Government Layout (Full-width, Custom Palette)
 * Palette: 252323 (dark), 70798c (blue-gray), f5f1ed (light), dad2bc (beige), a99985 (warm accent)
 * - No hero image
 * - Full-width bands
 * - Robust gallery that works with public/ images (spaces auto-encoded)
 */
...
/** Turn a filename or path into a browser-usable /public URL. */
function toPublicUrl(input) {
  if (!input) return input;
  // Normalize slashes first
  let s = String(input).replace(/\\/g, "/").trim();
  // If it's already an absolute http(s) or data URL, return as-is
  if (/^(https?:)?\/\//i.test(s) || s.startsWith("data:")) return s;
  // If it starts with '/', assume user already pointed to public path; encode safely
  if (s.startsWith("/")) return encodeURI(s);
  // Otherwise treat it as a file under /public/floods/
  return encodeURI(`/floods/${s}`);
}
...
function StatCard({ title, primary, facts }) {
  const p = percent(primary.part, primary.total); const { label, tone } = statusFromPercent(p);
  return (
    <div className="stat-card">
      <div className="stat-head"><div className="stat-title">{title}</div><StatusPill tone={tone}>{label}</StatusPill></div>
...
function ChartCard({ title, subtitle, children, legend }){
  return (
    <div className="chart-card">
      ...
      {legend?.length ? <div className="chart-legend">{legend.map((l,i)=><div key={i} className="legend-row"><span className={`swatch ${l.className||""}`} />{l.label}</div>)}</div> : null}
    </div>
  );
}

function LineChartPower(){
  const data = PSPCL; const target = 56; const W=460,H=190; const x = XScale(data.length, W); const y = YScale(Math.max(target, ...data.map(d=>d.restored)), H);
  const pts = data.map((d,i)=>`${x(i)},${y(d.restored)}`).join(" "); const ticks = [0, target/3, (2*target)/3, target];
  return (
...
function BarChartPatients(){
  const data = PATIENTS; const W=460,H=190; const left=36,right=12,top=10,bottom=24; const innerW=W-left-right; const bw = innerW/data.length - 8;
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="svgChart" role="img" aria-label="Patients attended bar chart">
      {[0,0.33,0.66,1].map((f,i)=>{const max=Math.max(...data.map(d=>d.patients)); const val=f*max; const yy=y(val); return <g key={i}><line x1="36" x2={W-12} y1={yy} y2={yy} className="gridline"/><text x="30" y={yy+3} className="tick">{Math.round(val)}</text></g>;})}
...
function ComboChartWater(){
  const { dates, affected, restored } = WATER; const W=460,H=190; const left=36,right=12,top=10,bottom=24; const innerW=W-left-right; const step = innerW/dates.length; const bw = (step-8)/2;
...
function SafeImg({ src, alt, ...rest }) {
  // Always build a correct public URL (encodes spaces etc.)
  const [url, setUrl] = React.useState(toPublicUrl(src));
  const [failed, setFailed] = React.useState(false);

  React.useEffect(() => {
    setUrl(toPublicUrl(src));
    setFailed(false);
  }, [src]);
  ...
  return (
    <img
      ...
      alt={alt}
      onError={() => {
        // simple fallback: flip extension case if needed
...
function FloodGallery({ images }) {
  ...
  React.useEffect(() => {
    ...
    const onKey = (e) => {
      ...
      if (e.key === "ArrowLeft")  setIdx(i => (i + images.length - 1) % images.length);
    ...
    <div className="page">
      <style>{`
        :root{
          /* Custom palette */
          --c-dark:#252323;
          --c-mid:#70798c;   /* blue-gray */
          --c-light:#f5f1ed; /* page bg */
          --c-beige:#dad2bc; /* band alt/bg */
          --c-warm:#a99985;  /* accent */
          --ink:#252323; --muted:#70798c;
          --ring:#e0dbd2; --card:#ffffff;
          --shadow:0 6px 20px rgba(37, 35, 35, .12);

          --good:#2e7d32; /* green, for clarity */
          --warn:#f9e0a8; /* light warning with dark text */
          --bad:#b71c1c;  /* deep red */
        }
        *{box-sizing:border-box}
        html,body,#__next,.page{height:100%}
        body{margin:0;background:var(--c-light)}
        .page{font-family: ui-sans-serif, system-ui, -apple-system, "Segoe UI", Roboto, Inter, Arial; color:var(--ink)}

        /* Bands */
        .band{width:100%; padding:24px 0}
        .band.alt{background:var(--c-beige); border-top:1px solid var(--ring); border-bottom:1px solid var(--ring)}
        .content{max-width:1400px; margin:0 auto; padding:0 24px}

        /* Header (no hero image) */
        .hero{padding:28px 0 8px; text-align:center}
        .title{font-weight:900; letter-spacing:.2px; color:var(--c-dark); font-size:40px}
        .lede{max-width:1000px; margin:12px auto 0; padding:14px 16px; border-radius:14px; background:#fff; color:var(--c-dark); border:1px solid var(--ring); box-shadow:var(--shadow)}
        .ctas{display:flex; justify-content:center; gap:12px; margin:16px 0 6px}
        .btn{display:inline-flex; align-items:center; gap:8px; border-radius:999px; padding:10px 18px; font-weight:700; border:1px solid var(--ring); background:#fff; color:var(--c-dark)}
        .btn.primary{background:var(--c-dark); color:#fff; border-color:transparent; box-shadow:var(--shadow)}
        .sub-link{font-size:14px; color:var(--c-dark); text-decoration:underline}
        .divider{height:1px;background:#e7e2d9;margin:18px 0}

        /* Cards / layout */
        .card{background:var(--card); border:1px solid var(--ring); border-radius:18px; padding:16px; box-shadow:var(--shadow); margin-top:14px}
        .card-title{font-weight:800;color:var(--c-dark);display:flex;align-items:center;gap:8px}
        ...
        .kpi{background:#fff;border:1px solid var(--ring);border-radius:14px;padding:16px;text-align:center;box-shadow:var(--shadow);position:relative;border-top:4px solid var(--c-warm)}
        .kpi-icon{width:40px;height:40px;border-radius:20px;background:var(--c-light);color:var(--c-dark);display:grid;place-items:center;margin:0 auto}
        .kpi-value{font-size:28px;font-weight:900;color:var(--c-dark);margin-top:8px}
        ...
        .list{margin:10px 0 0 18px;color:var(--c-dark)}
        ...
        .stat-title{font-weight:800;color:var(--c-dark)}
        .pill{padding:4px 10px;border-radius:999px;font-size:12px}
        .pill.good{background:var(--good); color:#fff}
        .pill.warn{background:var(--warn); color:#252323}
        .pill.bad{background:#f5c6c6; color:#252323}
        .stat-big{font-size:26px;font-weight:900;color:var(--c-dark)} .stat-small{font-size:12px;color:var(--muted);margin-left:6px}
        .stat-sub{font-size:12px;color:#444;margin-top:2px}
        .stat-bar{height:8px;background:#f0ece6;border-radius:6px;overflow:hidden;border:1px solid var(--ring);margin-top:8px}
        .stat-fill{height:8px;background:linear-gradient(90deg,var(--c-warm), var(--c-dark))}
        .stat-facts{margin:8px 0 0 18px;color:var(--c-dark)}
        ...
        .chart-title{font-weight:800;color:var(--c-dark)}
        .chart-sub{font-size:12px;color:var(--muted)}
        .chart-body{padding:0 4px}
        .chart-legend{display:flex;gap:16px;flex-wrap:wrap;padding:8px 6px 4px;color:var(--c-dark)}
        .legend-row{display:flex;align-items:center;gap:6px;font-size:12px}
        .swatch{width:12px;height:12px;border-radius:3px;display:inline-block;background:var(--c-warm)}
        .swatch.lgRef{background:transparent;border:2px dashed var(--c-mid);width:18px;height:10px;border-radius:6px}
        .swatch.lgA{background:rgba(169,153,133,.65)}
        .swatch.lgB{background:rgba(112,121,140,.25);border:1px solid rgba(112,121,140,.35)}
        .chart-note{grid-column:1/-1;font-size:12px;color:var(--muted);margin-top:-4px}
        .svgChart{width:100%;height:auto}
        .gridline{stroke:#e7e2d9;stroke-width:1}
        .tick,.xtick{font-size:10px;fill:var(--muted);text-anchor:end}
        .ref{stroke:var(--c-mid);stroke-dasharray:4 4;opacity:.7}
        .line{fill:none;stroke:var(--c-warm);stroke-width:2}
        .marker{fill:#fff;stroke:var(--c-warm);stroke-width:2}
        .barA{fill:rgba(169,153,133,.65)}
        .barB{fill:rgba(112,121,140,.25);stroke:rgba(112,121,140,.35);stroke-width:1}
        ...
        .thumb figcaption{font-size:12px;padding:8px 10px;color:var(--c-dark)}
        .lightbox{position:fixed;inset:0;background:rgba(37,35,35,.65);display:none;align-items:center;justify-content:center;z-index:60}
        ...
        .lb-cap{color:#252323;background:rgba(245,241,237,.9);padding:6px 10px;border-radius:8px;border:1px solid var(--ring)}
        .lb-close,.lb-nav{background:#fff;border:1px solid var(--ring);border-radius:999px;box-shadow:var(--shadow);color:var(--c-dark);cursor:pointer}
        ...
        .ph{display:grid;place-items:center;width:100%;height:160px;background:linear-gradient(135deg,#f5f1ed,#ebe5dc);border:1px dashed var(--ring)}
        .ph-cap{font-size:12px;color:#444}

        /* Footer */
        .footer{background:var(--c-dark);color:#f0ede9;text-align:center;padding:16px;margin-top:22px;border-top:4px solid var(--c-warm)}
      `}</style>
      ...
      <section className="band alt">
        <div className="content">
          <div className="grid kpi">{HEADLINES.map((h,i)=><KPI key={i} item={h} />)}</div>
        </div>
      </section>

      {/* ===== BAND: Overview (no map) ===== */}
      <section className="band">
        <div className="content">
            ...
            <div className="card">
              ...
              <ul className="list">
                <p>
                  ...
                  Nallah, caused flooding across border districts. The disaster affected 196 villages,
                  impacting over 1.36 lakh people. The government’s first priorities were restoring
                  electricity and potable water, followed by health, animal care, shelter, and road access.
                </p>
              </ul>
            </div>
            <div className="card">
              ...
              <ul className="list">
                ...
                <li><strong>District Emergency:</strong> 112</li>
                <li><strong>Relief & Shelter:</strong> BDPO / Tehsildar, SDO (PWD/WS&S)</li>
                <li><strong>Health:</strong> Mobile medical units; 108 ambulance</li>
              </ul>
            </div>
          </div>
          ...
          <div className="info-grid" style={{ marginTop: 16 }}>
            <div className="card">
              ...
              <ul className="list">
                <li><b>Power:</b> PSPCL restored supply to all reported affected habitations.</li>
                <li><b>Water & Sanitation:</b> Chlorination and de-watering in low-lying pockets; potable coverage expanding.</li>
                <li><b>Health:</b> Medical camps active; surveillance for water-borne diseases; essential medicines stocked.</li>
                <li><b>Animal Husbandry:</b> Veterinary teams treating livestock; fodder distribution coordinated with BDPOs.</li>
                <li><b>Shelter & Relief:</b> Dry ration and ready-to-eat kits; sleeping mats and blankets distributed.</li>
              </ul>
            </div>
            <div className="card">
              <div className="card-title">How You Can Help</div>
              <ul className="list">
                <li><b>Register:</b> NGOs should register assistance (water/sanitation, shelter, health, animal care, logistics).</li>
                <li><b>Coordinate:</b> Route activities via the District Control Room to target priority pockets.</li>
              </ul>
            </div>
          </div>
        </div>
      ...
      <section className="band alt">
        <div className="content">
          <div className="card" id="gallery">
            ...
            {/* IMPORTANT:
               - Put files inside /public/floods/
               - You can pass just the filename (with spaces) OR a web path beginning with /floods/ */}
            <FloodGallery images={[
              { src: "public/floods/flood images_page-0001.jpg", caption: "Flood situation image 1" },
              { src: "public/floods/flood images_page-0002.jpg", caption: "Flood situation image 2" },
              { src: "public/floods/flood images_page-0003.jpg", caption: "Flood situation image 3" },
              { src: "public/floods/flood images_page-0004.jpg", caption: "Flood situation image 4" },
              // If you prefer explicit paths, both also work:
              // { src: "/floods/flood images_page-0001.jpg", caption: "…" },
            ]}/>
          </div>
        </div>
      </section>
